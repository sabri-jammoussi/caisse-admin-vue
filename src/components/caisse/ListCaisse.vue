          ...
          <td>
            <router-link :to="`/EditCaisse/${item.id}`">
              <v-btn v-bind="attrs" v-on="on">
                ...
                <v-icon class="text-subtitle-1" color="blue" @click.stop="editCaisse(item.id)" style="margin-right: 12px">mdi-magnify</v-icon>
              </v-btn>
            </router-link>
            <v-btn v-bind="attrs" v-on="on" @click.stop="deleteCaisse(item.id)">
