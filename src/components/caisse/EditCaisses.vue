    ...
    <v-row justify="space-around">
      <v-card width="1000" title="Modifier">
        <v-sheet width="900" class="mx-auto">
          ...
          <v-btn v-if="!editMode" @click="enableEditMode" color="primary" class="button-group1">Modifier</v-btn>
          <v-btn v-else @click="submitChanges" color="success" class="button-group1">Submit</v-btn>
          <v-btn v-if="!editMode" color="red" @click="goToListCaisse" class="button-group">Retour</v-btn>
          <v-btn v-else color="red" @click="resetForm" class="button-group">Annuler</v-btn>
        </v-sheet>
      </v-card>
    </v-row>
  </v-container>
